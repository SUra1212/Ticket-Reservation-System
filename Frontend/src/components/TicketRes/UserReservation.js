...
export default function UserReservation() {
  ...
  useEffect(() => {
    axios.get(`https://localhost:7227/reservation`).then((res) => {
      console.log(res);
  ...
  const deleteTicketRes = (e, id) => {
    ...
    if (shouldDelete) {
      ...
      axios
        .delete(`https://localhost:7227/delete/${id}`)
        .then((res) => {
          console.log(res);
              ...
              {filteredTrainRes.map((item, index) => {
                return (
                  <tr key={index}>
                    ...
                    <th>{item.mobileNo}</th>
                    <th>{item.dateRes}</th>
                    <th>{item.noOfPassenger}</th>
                    <th>{item.class}</th>
                    <th>{item.total.toFixed(2)}</th>
                    <th>{item.status}</th>
                    <td>
                      {isEditable(item.dateRes) ? (
                        <Link to={`/edituserreservation/${item.id}`}>
                      ...
                      {isEditable(item.dateRes) ? (
                        <Link to={`/canceluserreservation/${item.id}`}>
                          <button className="btn btn-edit">
                            <i className="bi bi-pencil-square"></i>
                            Cancel
                          </button>
