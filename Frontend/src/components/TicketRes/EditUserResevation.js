...
export default function EditUserReservation() {
  let { id } = useParams();
...
//   }, [ticketres.trainName]);
  ...
  const handleInput = (e) => {
    ...
    // if (name === "class" || name === "noOfPassenger") {
    //   if (updatedTicketRes.class && updatedTicketRes.noOfPassenger) {
    //     let classPrice = 0;
  ...
  const UpdateTicketRes = (e) => {
    ...
    axios
      .put(`https://localhost:7227/update/${id}`, data)
      .then((res) => {
        notifyTicketUpdate();
        console.log(res);
        setTimeout(() => {
